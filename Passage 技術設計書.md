...
**KV incrementの非原子性について:** Workers KVのread-modify-writeは原子的ではなく、同一IPからの同時リクエストでレースコンディションが発生する可能性がある。レート制限の目的は厳密な流量制御ではなく、明らかな濫用の防止であるため、この設計上のトレードオフは許容範囲内である。厳密なレート制限が必要な場合は、Durable Objectsの使用を検討する。
...
チャンクテキスト2,000文字（約2KB） + 作品名200文字 + 著者名100文字 + その他フィールド200文字 = 合計約3KB。Vectorizeの10KiBメタデータ制限に対して十分な余裕がある。
